...
positionTooltip = ($el) ->
  ...
  position.top = switch direction
    ...
    when 'n' then position.top - $tooltip.outerHeight() - \
      (if hasOffsetTop then offTop else offset[1])
  ...
  position.left = switch direction
    ...
    when 'w' then position.left - $tooltip.outerWidth() - \
      (if hasOffsetLeft then offLeft else offset[0])
...
Meteor.startup ->
  $(document).on('mouseover', '[data-tooltip]:not([data-tooltip-trigger]),
    [data-tooltip-element]:not([data-tooltip-trigger]), [data-tooltip-trigger="hover"]',
    showTooltip)

  $(document).on('mouseout', '[data-tooltip]:not([data-tooltip-trigger]),
    [data-tooltip-element]:not([data-tooltip-trigger]), [data-tooltip-trigger="hover"]',
    hideTooltip)
